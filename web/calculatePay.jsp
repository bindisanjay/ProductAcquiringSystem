...
System.out.println("mmm::"+st);
  ...
  if(st.equalsIgnoreCase("sviews")) { 
      
      String id= request.getParameter("id");
      String salay= request.getParameter("salay");
      String nname= request.getParameter("nname");
          %>
                    <h4 class="card-title">View Payment Employee Name is: <%=nname%> </h4>
                    <div class="table-responsive">
                    <table class="table table-striped">
                     
  
                      <tbody>
                          <tr>  <th>  SID  </th> <td class="py-1"> <%=id%> </td>   </tr>
                          <tr>  <th>  Monthly Salary   </th> <td class="py-1"> <%=salay%> </td>   </tr>
                          
                          <%  int say=Integer.parseInt(salay);
                          System.out.println("salay ::"+say);
                          int day=say/30;  int hou=day/9;
                          System.out.println("day ::"+day);
                          System.out.println("hou ::"+hou);
           String sql=   "SELECT shiftname, count(shiftname) FROM empshifts where empid='"+id+"' and shiftname !='G' GROUP BY shiftname "; 
          Statement stmt=connect.createStatement();  ResultSet rs =stmt.executeQuery(sql); int utype=0;
       while (rs.next()) { utype+=Integer.parseInt(rs.getString(2));    }
              int shi=hou*6;  int mpaly=shi*utype;
                          System.out.println("shi ::"+shi);
                          System.out.println("mpaly ::"+mpaly);
               String sqlg=   "SELECT shiftname, count(shiftname) FROM empshifts where empid='"+id+"' and shiftname ='G' GROUP BY shiftname "; 
             ResultSet rsg =stmt.executeQuery(sqlg); int utypeg=0;
       while (rsg.next()) { utypeg+=Integer.parseInt(rsg.getString(2));    }
          int mpalyg=day*utypeg;
          int salary=mpaly+mpalyg;
          
                          System.out.println("mpalyg ::"+mpalyg);
                          System.out.println("salary ::"+salary);
              %>  
              
                          <tr>  <th>  Calculate Pay Salary   </th> <td class="py-1"> <%=salary%> </td>   </tr>
  </tbody>  </table>  </div>
<% }    }else{ %>
...
<tr>
  ...
  <td>  
  <form class="form-sample" action="calculatePay.jsp?st=sviews" method="post" > 
      <input type="hidden" class="form-control"  name="id" value="<%=rs.getString(1)%>"> 
      <input type="hidden" class="form-control"  name="salay" value="<%=rs.getString(10)%>"> 
      <input type="hidden" class="form-control"  name="nname" value="<%=rs.getString(2)%>"> 
    <input class="btn btn-block btn-primary btn-lg font-weight-medium auth-form-btn" type="submit" value="calculate Pay">
